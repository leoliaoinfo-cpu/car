...
function VehicleQuickReference() {
  ...
  const shown = compare.length > 0 ? VEHICLE_VARIANTS.filter((v) => compare.includes(v.id)) : variants;
  ...
  if (presentation) return <CustomerPresentation variants={shown} onClose={() => setPresentation(false)} />;
  return <section className="card p-4 md:p-6 space-y-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Kia K2500</h2><p className="text-xs text-ink-3 mt-1">3～5 秒找到售價與尺寸；所有數字來自同一份 Variant 資料。</p></div><button onClick={() => setPresentation(true)} className="btn-primary">給客戶看</button></div><div className="flex gap-2"><Chip active={drive === '2WD'} onClick={() => { setDrive('2WD'); setCompare([]); }}>2WD</Chip><Chip active={drive === '4WD'} onClick={() => { setDrive('4WD'); setCompare([]); }}>4WD</Chip>{drive === '2WD' && <><Chip active={transmission === '自排'} onClick={() => { setTransmission('自排'); setCompare([]); }}>自排</Chip><Chip active={transmission === '手排'} onClick={() => { setTransmission('手排'); setCompare([]); }}>手排</Chip></>}</div><div className="grid md:grid-cols-3 gap-3">{shown.map((v) => <VehicleCard key={v.id} variant={v} checked={compare.includes(v.id)} onCompare={() => toggleCompare(v.id)} />)}</div>{compare.length > 0 && <p className="text-xs text-ink-3">已選 {compare.length}/2 台比較；只呈現客觀差異，不做排名。</p>}</section>;
}

function VehicleCard({ variant: v, checked, onCompare, presentation = false }) {
  return <article className={`rounded-2xl ${presentation ? 'bg-white border border-slate-200' : 'bg-s2'} p-4 text-slate-800`}><div className="flex items-start justify-between"><div><p className="text-xs text-slate-500">{v.drive}・{v.transmissionLabel}</p><h3 className="text-lg font-bold mt-1">{v.cab}</h3></div>{!presentation && <label className="text-xs flex items-center gap-1"><input type="checkbox" checked={checked} onChange={onCompare} />比較</label>}</div><div className="mt-4"><p className="text-xs text-slate-500">售價</p><strong className="text-3xl tracking-tight">{formatVehiclePrice(v.msrpTwd)}</strong><p className="text-xs text-slate-400">{formatTwd(v.msrpTwd)}</p></div><div className="grid grid-cols-2 gap-3 mt-5"><Metric label="貨斗長" big={`${convertMmToTaiwaneseChi(v.cargoLengthMm)} 台尺`} small={`${v.cargoLengthMm.toLocaleString()} mm`} /><Metric label="車長" big={`${convertMmToTaiwaneseChi(v.lengthMm)} 台尺`} small={`${v.lengthMm.toLocaleString()} mm`} /><Metric label="車高" big={`${convertMmToTaiwaneseChi(v.heightMm)} 台尺`} small={`${v.heightMm.toLocaleString()} mm`} /><Metric label="載重" big={`${v.payloadKg.toLocaleString()} kg`} small="核定載重" /><Metric label="座位" big={`${v.seats} 人`} /><Metric label="平均油耗測試值" big={`${v.fuelEconomyKmL} km/L`} /></div><details className="mt-4"><summary className="text-sm font-semibold cursor-pointer">更多規格</summary><div className="mt-3 text-xs text-slate-600 space-y-1"><p>貨斗寬 {v.cargoWidthMm.toLocaleString()} mm</p><p>空車重 {v.curbWeightKg.toLocaleString()} kg・總重 {v.grossVehicleWeightKg.toLocaleString()} kg</p><p>排氣量 {v.displacementCc.toLocaleString()} c.c.</p><p>最大馬力 {v.maxPower}</p><p>最大扭力 {v.maxTorque}</p><p>最小迴轉半徑 {v.turningRadiusM} m</p>{v.differentialLock && <p>{v.differentialLock}</p>}</div></details></article>;
}

function Metric({ label, big, small }) { return <div><p className="text-[10px] text-slate-500">{label}</p><strong className="text-base block">{big}</strong>{small && <span className="text-[10px] text-slate-400">{small}</span>}</div>; }
