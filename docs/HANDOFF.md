...
> **2026-09-22 補充：** 已有車只委託改裝的客戶，報價可勾選「此單只報改裝，
> 不含車輛售價」；車型與配件選擇保留，但車價不計入總額、成本或 PNG。

> **2026-09-22 補充：** 客戶需求／用途是內部備忘，繼續保存並供業務編輯、回看；
> 客戶報價預覽與 PNG 出圖完全不顯示該文字。

> **2026-09-22 補充：** 升降尾門新增雙折尾門 3.5／4／4.5／5／5.5／6 尺，
> 全部待廠商報價；與一般尺寸尾門擇一，不改動原本固定價。舊選單升級僅補入新項。

> **2026-09-22 補充：** 「設定 → 報價選單 → 分類管理」可直接修改配備分類名稱；
> 改名連動分類順序、旗下配件及舊報價的「沒有選配」確認標記，並防止同名合併。

> **2026-09-22 進度：** 報價配備分類預設收起；逐類展開詢問後可勾選配件，或標記
> 「沒有選配」，分類才顯示已處理；儲存／輸出前會提醒未確認分類。設定可全螢幕管理
> 報價選單、展開全部分類、增設分類並調整順序；移動產品時不會跳到目的分類。
> 滑特規格尾門品名改為「升降尾門（尺數）」並歸在「滑特(升降尾門)」，其他尾門另列。
> 內部成本可記同配件多家供應商、各車型成本與優先標記；依價格顯示最低成本推薦，
> 未指派時用最高適用成本保守試算，簽約後可指派廠商並逐步核對交車 SOP。
> 初次使用改提示設定雲端同步，預設私人 repo 名為 `car-sales-data`，不與 TEST 共用；
> 舊連線設定不改。設定不再顯示備份還原入口，報價編輯亦移除四步驟進度列。
...
> 內部成本入口已完全移到「設定 → 內部業績與成本」，以固定提示「就讀的國小」
> 解鎖；中文答案欄使用可叫出中文輸入法的文字輸入並以圓點遮蔽。報價畫面沒有成本
> 入口，也不會跳出成本不足或低於成本提示。成本仍可逐項修改及合計，但只在設定區。
> 2026 配件表與 2025/11 隔熱紙表已補入選單，八折已逐筆換算為成本，隔熱紙業務價
> 依使用者定義為成本。貸款以 4.5% 概算，只顯示月付金、最高 84 期。
...
- 尚未設定雲端同步時顯示引導橫幅，前往設定連結汽車專用私人 repo
...
- 先填客戶需求，再選車型與配件；底板與升降尾門分區。尾門 2.5／3／3.5／4／4.5／5／5.5 尺各自有固定售價，6 尺與四缸特規為待報價；貨車導流板售價 3,500 元、內部成本 3,000 元。未確認價格不列入目前總額。
- 車身改色選項可填車色備註並輸出到客戶圖片；安卓環景主項勾選後才顯示 500 元中央置物盒加購。錏花板分為花紋鍍鋅鐵板與無花紋鍍鋅鋼平板兩種待報價選項。
- 設定 → 報價選單的配備以分類導覽與可展開區塊管理；可逐項改到既有／自訂分類，變更直接沿用在客戶與獨立報價。整單優惠範本標題為「優惠&折扣」。客戶圖片底部精簡為「售價、優惠折扣、最終專案價」，無致謝與頁尾小字。
- 內部報價試算每次開啟會保留手動實際成本，並以目前成本設定補齊缺漏後自動加總；未設定成本會逐項警示，可直接跳到成本設定搜尋與調整。每項另有優惠折扣欄，會即時重算折後售價與總利潤，儲存後同步寫回原報價。
- 型錄沒有的配件可在「額外配件／未列配件」輸入名稱及金額，或標成待確認金額。
- 內部成本只在設定頁的固定密碼入口後顯示；可逐項編輯售價、成本、優惠、折後價與利潤，並合計成本總額及優惠總額。
- 車體配備快選 chips（框式/篷式/冷凍廂/升降尾門/貨斗加高，含價格一鍵帶入）
- 優惠&折扣快選（汰舊換新、貨物稅減免等，報價單綠色顯示並自總價扣除）
- 車型、配備與整單優惠選單皆可在「設定→🚚報價選單」自訂名稱與金額
- 貸款概算：預設年利率 4.5%，最高 84 期；客戶只看到月付概算，不顯示配合公司及總利息，並明示實際條件以金融機構核定為準。
...
### ⚙️ 設定
分頁：☁️雲端同步（初次使用先連汽車專用私人 repo）、📈內部業績與成本（密碼保護）、🏷客戶分類、📶業務進度、✏️自訂欄位（日期型可設紀念日循環）、🏆業績欄位、📋待辦範本、🚚報價選單、⏱追蹤規則、📖使用說明。備份還原 UI 已移除，舊資料格式相容性仍保留；設定右上角可全螢幕展開與切換主題。
...
**業績表 / 安全**
- 業績與成本已移出主導覽，只能由「設定 → 內部業績與成本」輸入固定答案進入；提示為「就讀的國小」，不提供修改或取消密碼的介面。
...
**報價與成本中心已完成**：車價、型錄改裝及手動項目可新增多筆單項優惠，另有多筆整單優惠；客戶報價不顯示成本／利潤，也不跳成本警告。DB v7 新增 `pricingRecords`，只有設定內的密碼保護區提供成本設定、報價試算、缺成本與低於成本檢查。
