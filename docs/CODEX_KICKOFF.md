...
**專案**：貨車銷售業務（KIA 卡旺／K2500）用的工作系統 PWA。程式在 `assistant/`（React 18 + Vite + Tailwind），資料存瀏覽器 IndexedDB，透過使用者自己的 GitHub 私人 repo 同步（見 `assistant/src/sync.js`）。建置產出單一檔 `dist/index.html`（vite-plugin-singlefile），可直接雙擊使用。主導覽已有獨立報價工作區、待確認金額、額外配件、客戶需求與 PNG 輸出；內部成本只能從設定頁固定提示密碼入口進入；DB v9。
...
- 報價單／業績相關規矩：貸款用 4.5% 做概算，客戶只看月付、最高 84 期，不顯示貸款公司；成本／利潤絕不進客戶報價圖片，內部成本只從設定頁固定提示密碼入口進入；浮水印預設非業務名。
- 產品內容（車型/配件介紹）**以原廠圖片文字為準，不可自己編**。

**照片已決策**：不要新增需付款或維護的雲端照片服務。系統照片只存本機，跨裝置與客戶共享使用客戶既有 LINE 相簿；不可把照片塞進 GitHub `data.json`，也不要重新加入 Cloudflare R2，除非我日後明確改變決定。

其餘待辦與新需求，我會逐項告訴你。先讀完 HANDOFF、掌握現況，再依最新需求處理。
