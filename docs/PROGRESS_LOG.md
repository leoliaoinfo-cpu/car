...
## 2026-08-20 — 報價單項優惠、內部成本中心與 R2 照片同步

- 報價車價／型錄改裝／手動項目都可新增、修改、刪除多筆單項優惠，另保留多筆整單優惠；單項折後價不會低於零。
- 客戶版報價重做優惠視覺與五段總計，成本／利潤完全不進截圖；儲存與分享前加入缺成本、低於成本確認。
- DB v7 新增 `pricingRecords`；密碼區新增成本目錄、報價試算、單車預估／實際利潤與各配件利潤貢獻。
- DB v8 新增 R2 照片 metadata 與刪除佇列；支援離線續傳、跨裝置下載、本機快取與刪除同步，Blob 不進 GitHub `data.json`。
- 新增 `photo-worker/` Cloudflare Worker，包含 Bearer Secret、15 MB 限制、來源白名單、R2 CRUD 與自動測試。
