...
## 2026-09-22 — 已有車客戶可只報改裝

- 選車型仍自動帶入車價；需要只報改裝時可勾選「此單只報改裝，不含車輛售價」。
- 保留車型供配件與供應商成本比對，但車價不列入報價總額、成本快照或客戶圖片；取消勾選可恢復車價。儲存後重開仍保留此選擇。

## 2026-09-22 — 客戶需求改為內部備忘

- 客戶需求文字仍可在報價編輯區填寫、儲存與回看；獨立報價清單仍供業務查看。
- 客戶版報價預覽及 PNG 截圖區不再渲染該文字，避免把談話紀錄傳給客戶。

## 2026-09-22 — 3.5 尺以上雙折尾門待報價選項

- 升降尾門分類新增雙折尾門 3.5、4、4.5、5、5.5、6 尺各一項；全部標示待報價、不虛填售價或成本。
- 與一般尺寸尾門共用擇一群組，避免同一份報價誤選兩種尾門；舊報價選單升級時只補缺項，保留自行修改的價格與分類名稱。

## 2026-09-22 — 配備分類可直接改名

- 設定 → 報價選單 → 分類管理增加逐類「改名／儲存／取消」。改名會同步更新分類順序及該分類所有配件，不會留下同名空分類。
- 舊報價以原分類名稱記錄的「沒有選配」狀態會依改名對照顯示；重新儲存時採用新名稱。阻擋空白、重複及曾使用的舊名稱，避免舊報價分類混淆。

## 2026-09-22 — 報價分類確認、供應商成本與簽約交車核對

- 獨立與客戶報價的配備分類預設收合；逐類選配或勾選「沒有選配」才標成已處理，儲存／輸出時提醒仍未確認的分類。
- 設定可全螢幕展開報價選單、展開全部、新增與排序產品分類；改配件分類不再跳到目的分類，也保留空分類。
- 滑特規格尾門的品名去掉「滑特」，改歸獨立的「滑特(升降尾門)」分類，原有其他尾門仍在「升降尾門」。
- 內部成本支援同配件多家供應商及車型價格，最低成本僅作價格推薦、可手動設優先與指派；未指派先用適用的最高成本保守估算，缺成本仍警示。成交頁新增簽約到交車的 SOP 核對。
- 初次使用引導雲端同步，預設為汽車專用 `car-sales-data` 私人 repo，既有連線不變。移除備份還原 UI 與報價四步驟進度列，既有資料／資料庫相容性不刪除。

## 2026-09-22 — 報價選單分類與客戶圖片摘要精簡

- 設定的配備選單新增清楚的分類導覽與分組區塊，可按分類篩選、展開、新增配備，也可將單項移至既有或自訂分類；同一 `cat` 分類同步供客戶與獨立報價使用。
- 「補助折抵選單」更名為「優惠&折扣」。
- 客戶報價圖片底部只列「售價、優惠折扣、最終專案價」，移除致謝及頁尾小字；待廠商報價時仍保留「目前已確認金額」提示。

## 2026-09-22 — 成本自動帶入、即時計算與缺漏導引

- 每次開啟報價試算都重新比對目前成本設定；保留手動實際成本，缺漏項目自動帶入已設定的型錄成本。
- 成本未完整時仍即時計算「已填成本小計」，逐項標示缺漏並列出名稱。
- 型錄項目缺成本時可直接前往成本設定並搜尋該項；成本設定頁同步顯示未設定數量及警示底色。
- 每個報價項目在實際成本旁可直接輸入「優惠折扣」，即時連動折後售價、利潤貢獻、整張報價總價與總利潤；儲存後同步寫回客戶報價及圖片。

## 2026-09-21 — 車色備註、安卓二次加購與底板細分

- 車身改色項目加入車色備註欄，內容會保存並顯示於客戶預覽與輸出圖片。
- 勾選「安卓＋四錄＆環景＋專用底座」後才顯示「加購中央置物盒 500 元」；取消主項目會同步移除加購。
- 錏花板拆成花紋鍍鋅鐵板與無花紋鍍鋅鋼平板兩個待報價選項，舊名稱會自動升級。

## 2026-09-21 — 升降尾門尺寸完整拆分

- 將原本合併的 3～3.5、4～4.5、5～5.5 尺拆成六個獨立選項，各尺寸可單獨勾選與報價。
- 舊裝置已儲存的合併尺寸會自動拆開；若曾修改售價，拆分後兩個尺寸會保留該售價。

## 2026-09-21 — 精簡報價首頁並修正四缸尾門分類

- 移除獨立報價首頁的四步驟進度列。
- 四缸升降尾門統一歸入「升降尾門」；舊裝置已存於「客製車體」的項目會自動搬移。

## 2026-09-21 — 修正中文密碼輸入並完全隔離客戶報價與成本

- 內部成本入口改用可叫出中文輸入法、但畫面仍以圓點遮蔽的答案欄，固定答案可正常輸入。
- 報價儲存與圖片輸出不再跳出缺成本或低於成本訊息；所有成本檢查只留在設定內的密碼保護區。

## 2026-09-21 — 報價隱私收緊、額外配件與滑特尾門型錄

- 客戶貸款區只顯示月付概算，移除貸款公司與總利息，期數上限提高到 84 期。
- 成本入口從報價畫面移除，只能由設定頁輸入固定提示密碼進入；密碼修改區已移除。
- 新增「額外配件／未列配件」輸入區，可自行填名稱、金額或標記待確認金額。
- 貨斗底板與升降尾門拆分分類；新增 2.5～5.5 尺固定價、6 尺特規待報價、雙缸升級、四缸特規與貨車導流板。
- 貨車導流板更新為客戶售價 3,500 元、內部成本 3,000 元；舊的待報價預設會自動升級，手動修改值則保留。

## 2026-09-21 — 獨立報價、待廠商報價、隱藏成本試算與配件表

- 主導覽新增獨立報價工作區；DB v9 新增 `quoteDrafts`，支援需求、客戶快照、草稿搜尋與 PNG。
- 橡膠／錏花板／白鐵底板與四缸特製尾門可標成待廠商報價，不計入尚未確認的總額。
- 報價器新增密碼保護的內部成本抽屜；成本逐項可改，優惠明細同步編輯，自動加總成本、優惠、折後價與利潤，關閉即重新上鎖且不進客戶圖片。
- 匯入 2026 卡旺配件清單與 2025/11 隔熱紙速查表缺漏品項；八折逐筆換算成實際成本，隔熱紙業務價依使用者指示視為成本。長期成本仍可在成本設定修改。
- 貸款概算預設 4.5%，可標記三信／中租迪和，顯示月付與預估總利息並保留核貸免責。
- 計算測試 11 項全過、Vite 單檔正式建置成功，瀏覽器實際操作與 console 驗證無錯誤。
