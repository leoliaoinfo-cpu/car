...
> **2026-09-22 補充：** 報價配備分類預設收起，逐類選配或勾「沒有選配」才顯示已處理，漏問分類在儲存／輸出前提醒。設定可全螢幕、全部展開、新增及排序分類；配件改分類不跳頁。
> 內部成本可維護同配件多家供應商與各車型拿貨價、標優先及指派，未指派以適用最高成本試算；成交區有簽約→交車 SOP。
> 新使用者從雲端同步開始，預設汽車專用私人 repo `car-sales-data`；備份還原 UI 與報價四步驟進度列已移除。下文若有舊版資料庫或備份 UI 敘述，請以本補充及實際程式碼為準，**絕不可按下文舊測試指令刪除共用資料庫**。

> **2026-09-21 最新進度：** 主導覽已新增獨立「報價單」工作區，支援先填需求、
> 勾選車型／配件、額外配件、待確認金額、存草稿及輸出 PNG。成本入口只在
> 「設定 → 內部業績與成本」，固定以提示「就讀的國小」解鎖；報價畫面沒有成本
> 按鈕或成本警告；中文答案可正常輸入並以圓點遮蔽，成本永遠不進客戶預覽或圖片。
> DB v9 新增 `quoteDrafts`。
> 2026 卡旺配件表與隔熱紙表已補進共用選單；表中的八折已換算為實際成本，
> 隔熱紙「業務價（含稅）」依使用者定義為成本。底板與升降尾門已分區；尾門
> 2.5、3、3.5、4、4.5、5、5.5 尺各自有固定報價，6 尺與四缸特規為待報價；貨車導流板售價 3,500 元、內部成本 3,000 元。貸款以 4.5%
> 概算，只顯示月付金，最高 84 期，不顯示貸款公司。
> 車身改色可填顏色備註；安卓環景勾選後可再加購 500 元中央置物盒；貨斗底板已分成花紋錏花板與無花紋錏花平板。
> 內部報價試算會自動帶入成本設定並即時計算；缺少成本時逐項警示，可直接跳到成本設定調整。每項也可直接輸入優惠折扣，連動報價總價與總利潤並同步回客戶報價。
> 設定中的配備選單已可按分類管理及自訂分類；「優惠&折扣」管理整單優惠範本。客戶報價圖片底部只顯示售價、優惠折扣與最終專案價，不再顯示致謝頁尾。
...
- 報價單／業績相關規矩：貸款以 **4.5% 概算**且客戶只看月付金，最高 84 期；不得顯示貸款公司。內部成本與利潤不得進客戶圖片，且只能從設定的固定提示密碼入口進入；浮水印預設非業務名。
...
**報價與成本中心已完成**：報價項目及整張報價均可新增多筆優惠；客戶版不顯示成本／利潤或成本警告。只有設定內的密碼區提供成本設定、報價試算與單車利潤。

**資料庫相容性**：正式站曾短暫發布 DB v8，因此不能降版；目前 `DB_VERSION = 9`（獨立報價草稿），且不建立或同步 R2 照片資料。
