...
> 勾選車型／配件、標記待廠商報價、存草稿及輸出 PNG。報價器內另有與業績區
> 共用密碼的「內部成本」抽屜，可逐項編輯成本與優惠並自動計算成本總計、優惠總計、
> 折後利潤；成本永遠不進客戶預覽或圖片。DB v9 新增 `quoteDrafts`。
> 2026 卡旺配件表與隔熱紙表已補進共用選單；表中的八折已換算為實際成本，
> 隔熱紙「業務價（含稅）」依使用者定義為成本。貸款概算預設使用 4.5%，顯示
> 月付與預估總利息，並明示正式條件仍由三信／中租迪和核定。
...
- 報價單／業績相關規矩：貸款以 **4.5% 概算**並顯示月付、總利息與核貸免責；內部成本與利潤不得進客戶圖片，成本抽屜與業績區共用密碼；浮水印預設非業務名。
